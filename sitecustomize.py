...
def _market(asset):
    s=str(asset).upper()
    if re.search(r"(?:^|[_./-])OTC(?:$|[_./-])",s): return "OTC"
    if re.search(r"(?:^|[_./-])FLEX(?:$|[_./-])",s): return "FLEX"
    return None
...
def _publish(feed,found,source):
    ...
    for asset in sorted(new):
        ...
        if market:
            ...
            log.info("ASSET_MARKET_EXPLICIT asset=%s market=%s source=%s",asset,market,source)
...
def _patch():
    try: from market_feed import LiveMarketFeed
