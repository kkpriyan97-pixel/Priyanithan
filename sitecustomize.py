...
_BAD = {"FLEX", "FLEX TIME", "FLEX_TIME", "FIXED TIME", "FOREX", "STOCKS", "ASSET", "SYMBOL", "PAIR", "INSTRUMENT", "TICKER"}


def _candidate(v):
    if not isinstance(v, str): return None
    s = v.strip().upper()
    if s in _BAD or " " in s or not _PAIR_RE.fullmatch(s) or not any(c.isalpha() for c in s): return None
    return s
...
def _publish(feed, found, source):
    ...
    log.info("FLEX_ASSET_DISCOVERED source=%s new=%s total=%s assets=%s", source, len(new), len(feed.assets), sorted(new))
    for asset in sorted(new): feed.schedule_asset(asset)


def _attach(feed):
    if getattr(feed, "_resilient_asset_discovery_attached", False): return
    ...
    async def on_event_183(msg):
        ...
        if found:
            feed._flex_last_event = time.time(); _publish(feed, found, "authenticated_event_183")
    ...
    async def on_authenticated_event(msg):
        ...
        if event == 183: return
        found = set(); _extract(msg.get("d"), found)
        if found: _publish(feed, found, f"authenticated_event_{event}")
...
def _patch():
    ...
    def init(self, *args, **kwargs):
        original_init(self, *args, **kwargs); _attach(self)
