...
_PROFIT_KEYS = {
    "profitability", "profitability_percent", "profitability_percentage",
    "max_profitability", "max_profitability_percent", "payout", "payout_percent",
    "profit", "profit_percent", "winperc", "win_percent", "win_percentage",
}
    ...
    for key, value in node.items():
        ...
        if k in {"flex", "is_flex", "flex_enabled", "flex_available", "available_flex", "flex_time", "is_flex_time", "flextime"}:
            ...
            if isinstance(value, str) and value.strip().lower() in {
                "true", "1", "yes", "enabled", "available", "flex", "flex time", "flex_time", "flextime"
            }:
                return True
            ...
            if k in _ASSET_KEYS:
                ...
                for item in values:
                    if isinstance(item, dict):
                        item_flex = _has_flex_marker(item)
                        if node_flex or item_flex:
                            for subkey in (
                                "id", "pair", "symbol", "asset", "instrument", "p", "code",
                                "ticker", "short_name", "display_name", "symbol_name", "pair_name",
                            ):
