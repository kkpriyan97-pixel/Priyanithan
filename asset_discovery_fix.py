...
def _has_profitability(node):
    ...
    for key, value in node.items():
        if str(key).lower() not in _PROFIT_KEYS: continue
        try:
...
def _has_flex_marker(node):
    ...
    for key, value in node.items():
        ...
        if k in {"flex", "is_flex", "flex_enabled", "flex_available", "available_flex", "flex_time", "is_flex_time", "flextime"}:
            ...
            if isinstance(value, str) and value.strip().lower() in {"true", "1", "yes", "enabled", "available", "flex", "flex time", "flex_time", "flextime"}: return True
...
def _extract(node, found, flex_context=False, stats=None):
    ...
    if isinstance(node, dict):
        ...
        if local_flex: stats["qualified_nodes"] += 1
                ...
                for item in values:
                    if isinstance(item, dict):
                        for subkey in ("id", "pair", "symbol", "asset", "instrument", "p", "code", "ticker", "short_name", "display_name", "symbol_name", "pair_name"):
                            ...
                            if c: found.add(c); stats["candidates"] += 1
                    else:
                        c = _candidate(item)
                        if c: found.add(c); stats["candidates"] += 1
        ...
        stats["lists"] += 1
        for item in node: _extract(item, found, flex_context, stats)
    return stats
    ...
    async def hardened_start(self):
        async def enhanced_assets(msg):
            ...
            if found:
                ...
                samples = []
                if isinstance(payload, dict): samples = [str(k) for k in list(payload.keys())[:25]]
                log.info("FLEX_TIME_ASSET_SCAN event=%s dicts=%s lists=%s qualified_nodes=%s candidates=%s key_samples=%s", msg.get("e"), stats["dicts"], stats["lists"], stats["qualified_nodes"], stats["candidates"], samples)

        if FLEX_ONLY: self.assets.clear()
        ...
        async def request_discovery_events():
            ...
            for event_id in events:
                try: await self.client.send(98, [event_id], False)
                except Exception as exc: log.debug("FLEX_TIME_ASSET_EVENT_REQUEST_FAILED event=%s error=%s", event_id, type(exc).__name__)
        ...
        async def discovery_watch():
            while self.connected:
                try:
                    ...
                    if self.connected: await request_discovery_events()
