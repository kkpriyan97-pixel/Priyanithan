...
class OlympLiveFeed:
    ...
    def __init__(self,on_candle:Callable[[str,dict],None]):
        self.on_candle=on_candle; self.token=os.getenv("OLYMPTRADE_ACCESS_TOKEN","").strip()
        self.assets=[x.strip().upper() for x in os.getenv("OLYMPTRADE_ASSETS","ASIA_X").split(",") if x.strip()]
        self.enabled=bool(self.token); self.connected=False; self.thread=None; self.client=None; self.forming={}
    def start(self):
        ...
        try:
            ...
            for asset in self.assets:
                try:
                    ...
                    await self._history(history or []); log.info("Loaded Olymp 1m candle history: %s",asset)
    ...
    async def _history(self,h):
        ...
        for g in groups:
            ...
            for x in rows:
                ...
                try:
                    o=float(x.get("open",x.get("o"))); hi=float(x.get("high",x.get("h"))); lo=float(x.get("low",x.get("l"))); c=float(x.get("close",x.get("c"))); t=float(x.get("t",x.get("timestamp",x.get("time"))))
                except (TypeError,ValueError): continue
                if asset and hi>=max(o,c) and lo<=min(o,c): self.on_candle(asset,{"open":o,"high":hi,"low":lo,"close":c,"timestamp":t})
    ...
    def status(self):
        return {"configured":self.enabled,"connected":self.connected,"assets":self.assets,"mode":"READ_ONLY_MARKET_DATA","timeframe":"1m_from_live_ticks","auto_trade":False}
