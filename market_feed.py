...
class LiveMarketFeed:
    def __init__(self,on_candle):
        ...
        self.ticks=0; self.candles=0; self.connected=False; self._reconnect_task=None; self._poll_task=None; self._last_poll_log=0
    @staticmethod
        ...
        while self.connected:
            try:
                ...
                if now-self._last_poll_log>=30:
                    self._last_poll_log=now; log.info("FEED_HEARTBEAT connected=%s running=%s ticks=%s completed_1m=%s assets=%s",self.connected,self.client.running,self.ticks,self.candles,len(self.assets))
