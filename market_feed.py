...
class LiveMarketFeed:
    ...
    def _consume_history(self,asset,msg):
        for x in walk_candles(msg):
            p=normalize(x,asset)
            if p and p[0]==asset:
                self.history[asset].append(p[1]);self.history[asset]=deque(sorted(self.history[asset],key=lambda z:z["timestamp"])[-360:],maxlen=360)
    ...
    async def _poll_loop(self):
        while self.connected:
            for a in list(self.assets):
                try:self._consume_history(a,await self.client.request_candles(a,80))
