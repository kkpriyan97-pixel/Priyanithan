...
class LiveMarketFeed:
            ...
            if isinstance(x,dict):
                for k,v in x.items():
                    if str(k).lower() in {"pair","symbol","asset","instrument"}:
                        ...
                        for z in values:
                            s=str(z.get("id",z.get("pair",z.get("symbol",""))) if isinstance(z,dict) else z).upper().strip()
    ...
    def _put_candle(self,asset,candle,notify=False):
        ts=float(candle["timestamp"]);bucket=int(ts//60)*60;candle=dict(candle);candle["timestamp"]=float(bucket);old={x["timestamp"]:x for x in self.history[asset]};old[bucket]=candle;self.history[asset]=deque(sorted(old.values(),key=lambda z:z["timestamp"])[-360:],maxlen=360)
        if notify and bucket>self.last_completed[asset]:self.last_completed[asset]=bucket;self.candles+=1;self.on_candle(asset,dict(candle))
    def _consume_history(self,asset,msg):
        rows=[]
        for x in walk_candles(msg):
            p=normalize(x,asset)
            if p and p[0]==asset:rows.append(p[1])
        for candle in rows:self._put_candle(asset,candle,notify=False)
    async def _candle_response(self,msg):
        for x in walk_candles(msg):
            p=normalize(x)
            if not p:continue
            asset,candle=p;self.assets.add(asset);self._put_candle(asset,candle,notify=True)
