...
def _structure(d):
    ...
    # Market/candle evidence is the decision source. Indicators only add confirmation later.
    score=0; reasons=[]
...
def _learn(asset, pattern, regime, direction):
    try:
        ...
        # Compatible with old databases: only query columns known to exist.
...
def evaluate(asset,data,base):
    ...
    # A reversal candle can override short-term momentum only when it is clear.
    ...
    # Indicators are conformity only: they cannot create a direction.
    confirmations=0
    ...
    st=STATE[asset]
    # Only one signal for the same setup in the same 5-minute window.
    ...
    # Strong market/candle evidence is mandatory; indicators are never a veto by themselves.
    if abs(ms)<3:
