...
class BrainState:
    ...
    def is_in_cooldown(self,pair,now=None):
        ...
        u=float(self.cooldown_until.get(pair,0) or 0)
        if u<=now:
    ...
    def filter_candidates(self,assets,now=None):
        ...
        return [
            a for a in assets
            if a.get("pair")
            and not a.get("locked")
            and not a.get("locked_trading")
    ...
    def mark_signal_sent(self,**kw):
        ...
        if str(kw.get("strategy","")).upper()!=ALLOWED_STRATEGY:
            raise ValueError("Only AVWAP + Volume Profile strategy is allowed")
        ...
        s=ActiveSignal(
            ...
            pair=str(kw["pair"]),
            display_name=str(kw["display_name"]),
            direction=str(kw["direction"]).upper(),
            expiry_minutes=int(kw["expiry_minutes"]),
            entry_price=float(kw["entry_price"]),
            entry_ts=float(kw["entry_ts"]),
            scheduled_entry_ts=float(kw["scheduled_entry_ts"]),
            entry_candle_ts=kw["entry_candle_ts"],
            strategy=ALLOWED_STRATEGY,
            reason=str(kw.get("reason","")),
            confidence=int(kw.get("confidence",0)),
            pattern=str(kw.get("pattern","")),
            trend_15m=str(kw.get("trend_15m","")),
            structure_1m=str(kw.get("structure_1m","")),
            avwap=float(kw.get("avwap",0) or 0),
            poc=float(kw.get("poc",0) or 0),
            vah=float(kw.get("vah",0) or 0),
            val=float(kw.get("val",0) or 0),
            avwap_slope=float(kw.get("avwap_slope",0) or 0),
        )
    ...
    def finish_signal(self,key,exit_price,result_ts=None):
        ...
        rec={
            ...
            "entry_ts":s.entry_ts,
            "scheduled_entry_ts":s.scheduled_entry_ts,
            "result_ts":now,
            "strategy":s.strategy,
            "pattern":s.pattern,
    ...
    def prune_expired_cooldowns(self,now=None):
        ...
        for p,u in list(self.cooldown_until.items()):
            if float(u)<=now:
                self.cooldown_until.pop(p,None)


def rank_signal_candidates(candidates):
    """Rank only candidates emitted by the two-indicator live brain."""
    q=[
    ...
    return sorted(
        ...
        key=lambda x:(
            ...
            float(x.get("confluence_score") or x.get("market_quality") or 0),
