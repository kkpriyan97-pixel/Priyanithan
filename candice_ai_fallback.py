...
def install(app):
    """Multi-provider + multi-model failover. Any failed model moves to the next candidate."""
    if getattr(app, "_candice_ai_fallback_v7", False):
        return
    ...
    def _openai_call(url, key, model, snapshot, memory, strict=False, json_mode=False, timeout=8.0):
        ...
        if strict:
            prompt += "\nFINAL OUTPUT RULE: Return exactly one JSON object and nothing else. No markdown, code fence, or commentary."
        payload = {"model": model, "messages": [{"role": "user", "content": prompt}], "temperature": 0.1, "max_tokens": 400}
        if json_mode:
            payload["response_format"] = {"type": "json_object"}
        response = requests.post(url, headers={"Authorization": f"Bearer {key}", "Content-Type": "application/json"}, json=payload, timeout=timeout)
        if not response.ok:
    ...
    def _gemini_models():
        return _unique_models(_env_models("GEMINI_MODELS"), [os.getenv("GEMINI_MODEL", "").strip()], ["gemini-3.6-flash", "gemini-3.6-pro", "gemini-3.1-flash-lite", "gemini-3.1-pro"])

    def _groq_models():
        return _unique_models(_env_models("GROQ_MODELS"), [os.getenv("GROQ_MODEL", "").strip()], ["openai/gpt-oss-120b", "openai/gpt-oss-20b", "openai/gpt-oss-safeguard-20b", "qwen/qwen3.6-27b", "qwen/qwen3.8-27b", "groq/compound", "groq/compound-mini"])

    def _mistral_models():
        return _unique_models(_env_models("MISTRAL_MODELS"), [os.getenv("MISTRAL_MODEL", "").strip()], ["mistral-small-latest", "mistral-medium-latest", "mistral-large-latest", "magistral-small-latest", "magistral-medium-latest", "codestral-latest"])
    ...
    def _gemini_call(key, model, snapshot, memory, strict=False):
        ...
        response = requests.post("https://generativelanguage.googleapis.com/v1beta/interactions", headers={"x-goog-api-key": key, "Content-Type": "application/json"}, json=payload, timeout=12.0)
        if not response.ok:
            raise RuntimeError(f"HTTP {response.status_code}: {response.text[:300]}")
        data = response.json()
    ...
    def _rotate_openai(provider, url, key, models, snapshot, memory):
        failures = []
        if not key:
            return None, [f"{provider}:NOT_CONFIGURED"]
        for model in models:
            for attempt in (1, 2):
                try:
    ...
    def _rotate_gemini(key, models, snapshot, memory):
        failures = []
        if not key:
            return None, ["GEMINI:NOT_CONFIGURED"]
        for model in models:
            for attempt in (1, 2):
                try:
        ...
        for attempt in (1, 2):
            try:
                ...
                if normalized and "unavailable" not in reason and "not configured" not in reason:
                    engine.log.info("AI PRIMARY RECOVERED attempt=%s decision=%s confidence=%s expiry=%s", attempt, normalized["decision"], normalized["confidence"], normalized["expiry"])
                    return normalized
            except Exception as exc:
                engine.log.warning("AI primary attempt %s failed: %s", attempt, exc)
            if attempt == 1:
                time.sleep(0.4)
        return None
    ...
    def fallback_review(snapshot, memory):
        failures = []
        result, errs = _rotate_gemini(os.getenv("GEMINI_API_KEY", "").strip(), _gemini_models(), snapshot, memory)
        failures.extend(errs)
        if result:
            return result
        result, errs = _rotate_openai("GROQ", "https://api.groq.com/openai/v1/chat/completions", os.getenv("GROQ_API_KEY", "").strip(), _groq_models(), snapshot, memory)
        failures.extend(errs)
        if result:
            return result
        result, errs = _rotate_openai("MISTRAL", "https://api.mistral.ai/v1/chat/completions", os.getenv("MISTRAL_API_KEY", "").strip(), _mistral_models(), snapshot, memory)
        failures.extend(errs)
        if result:
            return result
    ...
    app._candice_ai_fallback_v7 = True
    app.log.info("CANDICE AI FALLBACK V7 ACTIVE — ALL PROVIDERS + ALL MODEL ROTATION — FAIL CLOSED")
