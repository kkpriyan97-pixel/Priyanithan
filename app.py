...
VERSION = '8.7-CANDICE-VISUAL-SIGNAL-CARD'
...
def build_signal_card(asset, direction, entry_ts, candle_ts, expiry, confidence, timeframe, evidence):
    ...
    f_title = _font(54, True); f_sub = _font(23, False); f_label = _font(22, True); f_big = _font(48, True)
    f_mid = _font(30, True); f_small = _font(21, False)
    d.rounded_rectangle((12, 12, W-12, H-12), radius=34, outline=(24, 125, 220), width=4)
    d.text((62, 45), 'CANDICE', font=f_title, fill=(245, 248, 255))
    d.text((365, 47), 'AI', font=f_title, fill=(30, 170, 255))
    d.text((65, 112), 'TRADING SIGNAL', font=f_sub, fill=(175, 200, 225))
    d.text((765, 52), 'DISCIPLINE', font=f_small, fill=(175, 200, 225))
    d.text((765, 80), 'CONFIRM • ALERT', font=f_small, fill=(175, 200, 225))

    d.rounded_rectangle((35, 165, W-35, 335), radius=28, fill=(8, 44, 50), outline=(25, 130, 130), width=3)
    d.text((75, 190), 'ASSET', font=f_label, fill=(140, 175, 195))
    asset_text = 'Asia Composite Index' if asset == 'ASIA_X' else asset
    af = _fit_text(d, asset_text, 860, 46, True)
    d.text((75, 225), asset_text, font=af, fill=(248, 250, 255))
    d.text((78, 290), asset, font=f_small, fill=(105, 175, 235))

    down = direction == 'DOWN'
    dir_fill = (190, 18, 42) if down else (8, 125, 70)
    dir_text = 'DOWN' if down else 'UP'
    arrow = '↓' if down else '↑'
    d.rounded_rectangle((35, 360, 405, 555), radius=26, fill=(28, 11, 30) if down else (7, 35, 29), outline=dir_fill, width=4)
    d.text((65, 388), 'TRADE DIRECTION', font=f_label, fill=(230, 150, 170) if down else (130, 215, 175))
    d.text((65, 438), arrow, font=_font(68, True), fill=dir_fill)
    d.text((145, 448), dir_text, font=f_big, fill=(255, 245, 248))

    cards = [(430, 'ENTRY TIME', datetime.fromtimestamp(entry_ts, UAE).strftime('%H:%M:%S UAE'), (25, 125, 220)),
             (650, 'EXPIRY', f'{expiry} MIN', (245, 175, 20)),
             (870, 'CANDLE', '1 MIN', (25, 190, 100))]
    for x, label, value, accent in cards:
        d.rounded_rectangle((x, 360, x+185, 555), radius=24, fill=(7, 25, 45), outline=accent, width=3)
        d.text((x+18, 388), label, font=_font(18, True), fill=(150, 180, 205))
        vf = _fit_text(d, value, 150, 30, True)
        d.text((x+18, 445), value, font=vf, fill=(248, 250, 255))

    d.rounded_rectangle((35, 580, 365, 710), radius=24, fill=(7, 25, 45), outline=(28, 105, 205), width=3)
    d.text((60, 602), 'HUMAN BRAIN', font=f_label, fill=(185, 150, 230))
    d.text((60, 650), 'CONFIRMED ✓', font=f_mid, fill=(80, 235, 150))
    d.rounded_rectangle((385, 580, 715, 710), radius=24, fill=(7, 25, 45), outline=(28, 145, 205), width=3)
    d.text((410, 602), 'AI ANALYSIS', font=f_label, fill=(155, 190, 230))
    d.text((410, 650), 'APPROVED ✓', font=f_mid, fill=(80, 235, 150))
    d.rounded_rectangle((735, 580, 1045, 710), radius=24, fill=(7, 25, 45), outline=(30, 125, 190), width=3)
    d.text((760, 602), 'CONFIDENCE', font=f_label, fill=(155, 190, 230))
    d.text((760, 646), f'{int(confidence)}%', font=_font(42, True), fill=(50, 235, 165))

    d.rounded_rectangle((35, 735, W-35, 1010), radius=26, fill=(5, 23, 45), outline=(18, 95, 165), width=3)
    d.text((65, 762), 'SIGNAL REASONS', font=f_mid, fill=(155, 195, 235))
    items = list(evidence or ())[:4]
    if not items: items = ['Multi-indicator alignment verified']
    y = 820
    for i, item in enumerate(items):
        d.ellipse((68, y+6, 86, y+24), fill=[(40, 220, 135), (30, 165, 235), (245, 175, 25), (170, 70, 235)][i % 4])
        lines = _wrap_lines(d, item, 900, f_small)
        for line in lines[:2]:
            d.text((110, y), line, font=f_small, fill=(235, 242, 250)); y += 31
        y += 10

    d.rounded_rectangle((35, 1035, W-35, 1170), radius=26, fill=(48, 8, 35), outline=(225, 35, 70), width=4)
    d.text((70, 1060), '⏰  ALERT', font=f_mid, fill=(255, 105, 125))
    d.text((70, 1110), f'{PRE_ENTRY_SECONDS} SEC BEFORE ENTRY', font=_font(32, True), fill=(250, 250, 255))
    d.text((680, 1068), 'GET READY', font=f_mid, fill=(255, 125, 150))
    d.text((680, 1110), 'MANUAL EXECUTION', font=f_small, fill=(235, 190, 205))

    d.text((55, 1205), '🛡️ MANUAL TRADE ONLY', font=f_mid, fill=(65, 235, 155))
    d.text((55, 1245), 'AUTO-TRADE OFF  •  DEMO MODE', font=f_small, fill=(150, 185, 215))
    d.text((735, 1208), str(timeframe), font=f_small, fill=(90, 185, 240))
    d.text((735, 1245), datetime.fromtimestamp(candle_ts, UAE).strftime('%H:%M:%S UAE'), font=f_small, fill=(125, 160, 190))
...
async def send_signal_card(cid, asset, direction, entry_ts, candle_ts, expiry, confidence, timeframe, evidence):
    ...
    try:
        ...
        log.info('TELEGRAM SIGNAL CARD SENT chat=%s message_id=%s asset=%s direction=%s expiry=%s confidence=%s', cid, m.message_id, asset, direction, expiry, confidence)
        return m
    except Exception as e:
        log.exception('TELEGRAM SIGNAL CARD FAILED: %s', e); return None
...
async def scan_once():
    ...
    for uid, asset in list(selected.items()):
        ...
        if msg is None:
            log.warning('SIGNAL NOT RESERVED pair=%s reason=telegram card delivery failed', asset); continue
...
async def bot_main():
    ...
    if webhook_base:
        ...
        log.warning('CANDICE TELEGRAM ONLINE — WEBHOOK MODE — %s — 1M SCANNER — VISUAL SIGNAL CARDS — 10S PRE-ENTRY TIMER — AUTO-TRADE OFF', webhook_url)
    else:
        await tg_app.bot.delete_webhook(drop_pending_updates=True)
        await tg_app.updater.start_polling(drop_pending_updates=True)
        log.warning('CANDICE TELEGRAM ONLINE — LOCAL POLLING MODE — 1M SCANNER — VISUAL SIGNAL CARDS — 10S PRE-ENTRY TIMER — AUTO-TRADE OFF')
