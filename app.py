...
def normalize_candles(raw):
    ...
    for c in raw:
        ...
        if None in (o, h, l, cl):
            continue
        try:
            rows.append({"timestamp": float(ts) if ts is not None else time.time(), "open": float(o), "high": float(h), "low": float(l), "close": float(cl), "volume": float(c.get("volume", c.get("v", 0)) or 0)})
...
async def on_trade_event(message):
    ...
    for item in data:
        ...
        if not trade_id:
            continue
        manual_trades.setdefault(str(trade_id), {}).update({"event": event, "data": item, "updated_at": time.time()})
...
def analyze_pair(pair, df):
    ...
    ema9 = EMAIndicator(close, window=9).ema_indicator(); ema21 = EMAIndicator(close, window=21).ema_indicator()
    macd_obj = MACD(close); macd = macd_obj.macd(); macd_signal = macd_obj.macd_signal()
    rsi = RSIIndicator(close, window=14).rsi(); stoch = StochasticOscillator(high, low, close).stoch(); adx = ADXIndicator(high, low, close).adx()
    bb = BollingerBands(close); bb_high = bb.bollinger_hband(); bb_low = bb.bollinger_lband()
    c, prev = float(close.iloc[-1]), float(close.iloc[-2]); e9, e21 = float(ema9.iloc[-1]), float(ema21.iloc[-1])
    m, ms, rv, sv, av = float(macd.iloc[-1]), float(macd_signal.iloc[-1]), float(rsi.iloc[-1]), float(stoch.iloc[-1]), float(adx.iloc[-1])
    bh, bl = float(bb_high.iloc[-1]), float(bb_low.iloc[-1]); score_up = score_down = 0; reasons=[]; patterns=[]
    if e9 > e21: score_up += 2; reasons.append("EMA bullish")
    elif e9 < e21: score_down += 2; reasons.append("EMA bearish")
    if m > ms: score_up += 2; reasons.append("MACD bullish")
    elif m < ms: score_down += 2; reasons.append("MACD bearish")
    if rv >= 52 and rv < 70: score_up += 1
    elif rv <= 48 and rv > 30: score_down += 1
    if c > bh: score_down += 1; reasons.append("above upper BB")
    elif c < bl: score_up += 1; reasons.append("below lower BB")
    if sv > 80: score_down += 1; reasons.append("stoch overbought")
    elif sv < 20: score_up += 1; reasons.append("stoch oversold")
    if av >= 25:
        if score_up > score_down: score_up += 2
        elif score_down > score_up: score_down += 2
    elif av >= 20:
        if score_up > score_down: score_up += 1
        elif score_down > score_up: score_down += 1
    body=abs(c-prev)
    if body > 0: patterns.append("bullish close" if c > prev else "bearish close")
    ...
    if av < 15: confidence=min(confidence,62)
    return {"pair":pair,"signal":signal,"confidence":confidence,"candle_time":format_uae_timestamp(float(df["timestamp"].iloc[-1])),"patterns":patterns,"trend":"BULLISH" if score_up>score_down else ("BEARISH" if score_down>score_up else "MIXED"),"rsi":rv,"adx":av,"reason":"; ".join(reasons[:5]) or "mixed indicators","price":c,"ema9":e9,"ema21":e21,"macd":m,"macd_signal":ms,"stoch":sv,"bb_high":bh,"bb_low":bl}
...
Evaluate price action, candle patterns, market structure, EMA, MACD, RSI, Bollinger Bands, Stochastic, ADX, support/resistance and conflicts.
Rules:
...
- A single caution is not by itself enough to reject.
...
Return JSON only: {{"decision":"APPROVE|REJECT","direction":"UP|DOWN|NO SIGNAL","confidence":0-100,"duration_min":1|2|3|5|10|15,"reason":"short reason"}}
DATA:
{json.dumps(result, ensure_ascii=False)}""".strip()

def _json_object_from_text(text):
    text=str(text or "").strip();
    if not text: raise ValueError("AI response contained no text content")
    text=re.sub(r"^\s*```(?:json)?\s*", "", text, flags=re.I); text=re.sub(r"\s*```\s*$", "", text)
    try:
...
def parse_ai(data):
    if not isinstance(data,dict): raise ValueError("AI response is not a JSON object")
    choices=data.get("choices")
    if not choices or not isinstance(choices[0],dict): raise ValueError(f"AI response missing choices: {str(data)[:500]}")
    choice=choices[0]; msg=choice.get("message") if isinstance(choice.get("message"),dict) else {}
    ...
    for value in values:
        if isinstance(value,list): value="".join(str(x.get("text") or x.get("content") or "") if isinstance(x,dict) else str(x) for x in value)
        if not value: continue
        if isinstance(value,dict) and "decision" in value: return value
        if isinstance(value,str):
            try:
                obj=json.loads(value)
                if isinstance(obj,dict) and "decision" in obj: return obj
            except: pass
            for match in re.finditer(r'\{',value):
                try: obj,_=json.JSONDecoder().raw_decode(value[match.start():])
                except: continue
                if isinstance(obj,dict) and "decision" in obj: return obj
    raise ValueError("AI response contained no valid decision JSON")

def call_ai(prompt):
    log.info("AI FALLBACK CHAIN START: prompt_chars=%s",len(str(prompt))); providers=[]
    if OPENROUTER_API_KEY:
        models=[]
        for m in (OPENROUTER_MODEL,"openrouter/free","minimax/minimax-m3:free","google/gemma-4-26b-a4b-it:free"):
            if m and m not in models: models.append(m)
        for m in models: providers.append((f"OpenRouter/{m}","https://openrouter.ai/api/v1/chat/completions",OPENROUTER_API_KEY,m))
    groq_key=os.getenv("GROQ_API_KEY")
    if groq_key: providers.append(("Groq","https://api.groq.com/openai/v1/chat/completions",groq_key,os.getenv("GROQ_MODEL","openai/gpt-oss-120b")))
    cerebras_key=os.getenv("CEREBRAS_API_KEY")
    if cerebras_key: providers.append(("Cerebras","https://api.cerebras.ai/v1/chat/completions",cerebras_key,os.getenv("CEREBRAS_MODEL","llama3.1-8b")))
    mistral_key=os.getenv("MISTRAL_API_KEY")
    if mistral_key: providers.append(("Mistral","https://api.mistral.ai/v1/chat/completions",mistral_key,os.getenv("MISTRAL_MODEL","mistral-small-latest")))
    if AIRFORCE_API_KEY: providers.append(("Airforce","https://api.airforce/v1/chat/completions",AIRFORCE_API_KEY,AIRFORCE_MODEL))
    if not providers: return None,"No AI provider configured"
    schema={"type":"object","properties":{"decision":{"type":"string","enum":["APPROVE","REJECT"]},"direction":{"type":"string","enum":["UP","DOWN","NO SIGNAL"]},"confidence":{"type":"integer","minimum":0,"maximum":100},"duration_min":{"type":"integer","enum":[1,2,3,5,10,15]},"reason":{"type":"string"}},"required":["decision","direction","confidence","duration_min","reason"],"additionalProperties":False}
    last_error=None
    for index,(name,url,key,model) in enumerate(providers,1):
        try:
            headers={"Authorization":f"Bearer {key.strip()}","Content-Type":"application/json"}
            if name.startswith("OpenRouter/"): headers["HTTP-Referer"]="https://priyanithan-ai.onrender.com"; headers["X-Title"]="Priyanithan AI OlympTrade Signal Bot"
            payload={"model":model,"messages":[{"role":"system","content":"Return exactly one valid JSON object and no reasoning/prose/markdown."},{"role":"user","content":prompt}],"temperature":0,"max_tokens":300,"response_format":{"type":"json_object"}}
            r=requests.post(url,headers=headers,json=payload,timeout=20)
            if not r.ok: raise RuntimeError(f"HTTP {r.status_code}: {r.text[:500]}")
            parsed=parse_ai(r.json()); return parsed,None
        except Exception as e:
            last_error=f"{name}: {e}"; log.warning("AI PROVIDER FAILED: %s",last_error)
    return None,last_error or "All AI providers failed"

def normalize_ai_decision(ai):
    if not isinstance(ai,dict): return ai
    raw=str(ai.get("decision","")).strip().upper()
    if raw in ("APPROVE","REJECT"): ai["decision"]=raw; return ai
    ai["decision"]="REJECT"; ai["decision_error"]=f"Invalid AI decision '{raw or 'EMPTY'}'; approval not assumed."; return ai
...
def format_signal(result,ai=None):
    ...
    if result["signal"]=="NO SIGNAL" or not ai: return f"🚫 NO SIGNAL\n\n📈 {result['pair']}\n🕐 {result['candle_time']}\n\n🕯️ Patterns: {', '.join(result['patterns']) or 'None'}\n📈 Trend: {result['trend']}\n📊 RSI: {result['rsi']:.1f}\n💪 ADX: {result['adx']:.1f}\n\n⚠️ {result['reason']}\n⏳ Waiting for stronger setup..."
    decision=str(ai.get("decision","REJECT")).upper(); direction=str(ai.get("direction","NO SIGNAL")).upper()
    try: conf=int(ai.get("confidence",0))
    except: conf=0
    if decision!="APPROVE" or direction not in ("UP","DOWN") or direction!=result["signal"] or conf<AI_MIN_CONFIDENCE: return f"🚫 NO SIGNAL\n\n📈 {result['pair']}\n🕐 {result['candle_time']}\n\n📊 Technical: {result['signal']} ({result['confidence']}%)\n🤖 AI: {decision} / {direction} / {conf}%\n⚠️ {ai.get('decision_error',ai.get('reason','AI rejected the setup.'))}\n\n⏳ Waiting for stronger confirmation..."
    fire="🔥🔥🔥" if conf>=94 else ("🔥🔥" if conf>=91 else "🔥"); arrow="⬆️" if direction=="UP" else "⬇️"; duration=f"{choose_duration_min(result,ai)} MIN"
...
async def scan_cycle(application):
    ...
    candidates=[]; candle_ready=0; candle_failed=0; technical_no_signal=0
    for pair in universe:
        df,err=await get_ot_candles(pair,60,120)
        if df is None: candle_failed+=1; continue
        candle_ready+=1
        try: result=analyze_pair(pair,df)
        except Exception as e: log.warning("Technical analysis failed pair=%s: %s",pair,e); continue
        if result["signal"]!="NO SIGNAL": candidates.append(result)
        else: technical_no_signal+=1
    candidates.sort(key=lambda x:x["confidence"],reverse=True); checked=candidates[:MAX_AI_CANDIDATES]; approved=[]
    for result in checked:
        ai,err=call_ai(ai_prompt(result))
        if err: continue
        ai=normalize_ai_decision(ai); direction=str(ai.get("direction","NO SIGNAL")).upper()
        try: conf=int(ai.get("confidence",0))
        except: conf=0
        if ai.get("decision")=="APPROVE" and direction==result["signal"] and conf>=AI_MIN_CONFIDENCE:
            approved.append((result,ai))
            if len(approved)>=MAX_SIGNALS_PER_CYCLE: break
    if approved:
        for result,ai in approved: await send_to_recipients(application.bot,format_signal(result,ai))
    else:
...
async def access_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0].strip()==ACCESS_CODE:
        user=getattr(update,"effective_user",None)
        if user:
            authorized_users.add(int(user.id)); await update.message.reply_text("✅ Access approved. This chat can receive signals.")
        return
...
async def manual_trade_monitor(application):
    while True:
        try:
            client=ot_client; account_id=getattr(client,"account_id",None) if client else None
            if client and client.connection.is_connected and account_id:
                try:
                    ...
                    if isinstance(trades,list):
                        for t in trades:
                            if isinstance(t,dict) and t.get("id"): manual_trades.setdefault(str(t["id"]),{})["open"]=t
                except Exception as e: log.debug("Open-trade read unavailable: %s",e)
        except Exception: log.exception("Manual monitor error")
...
async def telegram_runtime(application):
    ...
    runtime_loop=asyncio.get_running_loop(); telegram_application=application
    await application.initialize(); await application.start()
    tasks=[asyncio.create_task(olymptrade_connect_loop(),name="olymptrade-connect"),asyncio.create_task(scan_loop(application),name="signal-scan"),asyncio.create_task(manual_trade_monitor(application),name="trade-monitor")]
    try:
        webhook_base=os.getenv("RENDER_EXTERNAL_URL","https://priyanithan-ai.onrender.com").rstrip("/"); webhook_url=f"{webhook_base}/telegram/webhook"
...
def main():
    ...
    application.add_handler(CommandHandler("start",start_cmd)); application.add_handler(CommandHandler("access",access_cmd)); application.add_handler(CommandHandler("scan",scan_cmd))
    ot_client=OlympTradeClient(OLYMPTRADE_ACCESS_TOKEN,parameters)
    asyncio.run(telegram_runtime(application))

def start_web_server():
    port=int(os.getenv("PORT","10000")); log.info("Render health server starting on 0.0.0.0:%s",port); app.run(host="0.0.0.0",port=port,debug=False,use_reloader=False)

if __name__=="__main__":
    threading.Thread(target=start_web_server,daemon=True,name="render-health").start(); main()
