    ...
    while True:
        try:
            ...
            for u in r.json().get("result", []):
                ...
                chat = m.get("chat") or {}
                cid = str(chat.get("id", ""))
                if sender.get("is_bot") or not cid:
                    continue
                text = str(m.get("text", "")).strip()
                if text.lower().startswith("/start"):
                    active_chat = cid
                    telegram_send("🎯 CANDICE AI\n\n✅ ONLINE\n📡 LIVE MARKET • READ-ONLY\n🚫 AUTO-TRADE • OFF\n🚫 MARTINGALE • OFF")
                    log.info("TELEGRAM_CHAT_AUTHORIZED chat=%s", cid)
        except Exception as e:
            log.warning("Telegram poll error: %s", type(e).__name__)
            time.sleep(5)


def on_history(asset: str, candle: dict):
    with lock:
        q = candles.setdefault(asset, deque(maxlen=360))
        ts = float(candle.get("timestamp", 0))
        if not q or ts > float(q[-1].get("timestamp", -1)):
            q.append(dict(candle))


def on_candle(asset: str, candle: dict):
    on_history(asset, candle)
    brain.on_completed_candle(asset, candle, candles, live_feed, telegram_send)
...
def start():
    ...
    if live_feed is not None:
        return
    live_feed = OlympLiveFeed(on_candle=on_candle, on_history=on_history)
    live_feed.start()
