...
INTERVAL = max(60, int(os.getenv('SCAN_INTERVAL_SECONDS', '60')))
...
async def send_text(cid, text, reply_markup=None):
    ...
    try:
        ...
        log.info('TELEGRAM TEXT SENT chat=%s message_id=%s', cid, m.message_id)
        return m
    except Exception as e:
        log.exception('TELEGRAM TEXT FAILED: %s', e)
        return None

async def edit_text(cid, mid, text, reply_markup=None):
    try:
        await tg_app.bot.edit_message_text(chat_id=cid, message_id=mid, text=text, reply_markup=reply_markup, disable_web_page_preview=True)
        return True
    except Exception as e:
        log.warning('TELEGRAM TEXT EDIT FAILED message_id=%s: %s', mid, e)
        return False
...
async def cmd_status(update, ctx):
    ...
    await send_text(uid, f'{header("SYSTEM STATUS")}\n\n'
        f'🛰️ Broker: {"🟢 CONNECTED" if broker.connected() else "🔴 DISCONNECTED"}\n'
        f'📈 FLEX: {selected.get(uid, "NONE")}\n'
        f'🔄 Scanner: every 1 minute\n'
...
async def cmd_session(update, ctx):
    ...
    await send_text(update.effective_user.id, f'{header("LIVE SCAN CONTROL")}\n\n'
        f'🟢 CONTINUOUS SIGNAL ENGINE\n'
        f'🔄 Analysis: every 1 minute\n'
        f'🕯️ Candle: closed 1M\n'
        f'⏱️ Expiry: 2 / 3 / 5 / 15 MIN\n'
        f'⏳ Signal countdown: ACTIVE\n'
        f'🏁 Expiry verification: ACTIVE\n\n'
        f'🔬 Research: 24/7\n🛡️ Risk gates: ACTIVE\n'
        f'🚫 Auto-trade OFF • Manual only')
...
async def asset_callback(update, ctx):
    ...
    await q.answer()
    uid = q.from_user.id
    if uid not in users or not q.data.startswith('asset:'):
        return
...
async def result_monitor(uid, s, msg):
    ...
    await send_text(uid, f'{header("EXPIRY VERIFICATION")}\n\n'
        ...
        f'🔎 Waiting for the boundary candle to close...')
    row = None
    err = ''
    for _ in range(8):
    ...
    if row is None:
        active.pop(uid, None)
        await send_text(uid, f'{header("RESULT UNRESOLVED")}\n\n'
            f'📈 {s.asset}\n💰 Entry • {s.entry:.6f}\n🏁 Expiry • —\n'
            f'🕒 Boundary • {datetime.fromtimestamp(end, UAE).strftime("%H:%M:%S UAE")}\n\n'
            f'⚠️ {err or "No verified closed expiry candle"}\n🚫 No WIN/LOSS recorded')
        return
    ...
    await send_text(uid, f'{header("FINAL MARKET OUTCOME")}\n\n'
        ...
        f'🔥 Loss streak • {daily["streak"]}/{MAX_STREAK}\n\n'
        f'MARKET OUTCOME • NOT BROKER ACCOUNT P/L\n🚫 AUTO-TRADE OFF • MANUAL ONLY')
    if result == 'LOSS':
        ...
        await send_text(uid, f'{header("RECOVERY WINDOW")}\n\n'
            ...
            f'🔬 Research continues\n🚫 Martingale OFF • Auto-trade OFF')

async def scan_once():
    reset_daily()
    if daily['losses'] >= MAX_DAILY_LOSSES or daily['streak'] >= MAX_STREAK:
        log.warning('CANDICE RISK STOP losses=%s streak=%s', daily['losses'], daily['streak'])
        return
    assets = await broker.live_assets()
    ...
    async def research(asset):
        async with sem:
            ...
            if e is None and df is not None and not df.empty:
                ...
                record({'ts': time.time(), 'asset': asset, 'research': True, 'direction': snap['direction'], 'strength': snap['strength'], 'rsi': snap['rsi14'], 'adx': snap['adx14']})
                ...
                log.info('CANDICE RESEARCH REJECT pair=%s reason=%s', asset, e or 'no fresh candles')
    ...
    for uid, asset in list(selected.items()):
        ...
        if row is None:
            log.info('ENTRY REJECT pair=%s reason=no closed 1m candle', asset)
            continue
        ...
        if key in sent_keys:
            continue
        ts = time.time()
        s = Signal(asset, a.direction, a.confidence, a.expiry, entry, ts, candle_ts, a.reason, a.evidence)
        active[uid] = s
        sent_keys.add(key)
        ...
        text = (f'{header("NEW SIGNAL")}\n\n📈 {asset}\n\n'
                ...
                f'🕒 Entry • {datetime.fromtimestamp(ts, UAE).strftime("%H:%M:%S UAE")}\n'
                f'🕯️ Candle • {datetime.fromtimestamp(candle_ts, UAE).strftime("%H:%M:%S UAE")}\n\n'
...
async def bot_main():
    ...
    for command, fn in [('start', cmd_start), ('access', cmd_access), ('assets', cmd_assets), ('status', cmd_status), ('session', cmd_session), ('update', cmd_update)]:
...
@flask_app.get('/')
def home():
    return f'{VERSION} ONLINE — FLEX market-data / 1-minute manual signals / timers active'
