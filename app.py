...
def session_window():
    ...
    if u < signal_end: start,end=base,signal_end; active=True
    else: start,end=signal_end,block_end; active=False
    next_start=block_end
    return start.astimezone(UAE),end.astimezone(UAE),next_start.astimezone(UAE),active

def session_payload():
    start,end,next_start,active=session_window(); remaining=max(0,(end-now()).total_seconds()); total=max(1,(end-start).total_seconds())
    return dict(active=active,session_start=start.strftime('%Y-%m-%d %H:%M:%S UAE'),session_end=end.strftime('%Y-%m-%d %H:%M:%S UAE'),next_session=next_start.strftime('%Y-%m-%d %H:%M:%S UAE'),remaining=int(remaining),total_seconds=int(total))
...
async def send_image(cid,kind='selected',asset='ASIA_X',direction='',confidence=0,expiry=0,reason='',reply_markup=None,caption='',**kwargs):
    if tg_app is None:
        log.warning('TELEGRAM IMAGE SKIPPED: bot not initialized kind=%s asset=%s',kind,asset); return None
    try:
        ...
        log.info('TELEGRAM IMAGE SENT kind=%s asset=%s message_id=%s',kind,asset,msg.message_id)
        return msg
    except Exception as e:
        log.exception('TELEGRAM IMAGE FAILED kind=%s asset=%s: %s',kind,asset,e); return None
...
async def edit_image(cid,message_id,kind,asset='ASIA_X',direction='',confidence=0,expiry=0,reason='',caption='',**kwargs):
    try:
        ...
        await tg_app.bot.edit_message_media(chat_id=cid,message_id=message_id,media=InputMediaPhoto(media=payload,filename='candice-update.png',caption=caption or 'CANDICE AI • Live Market • Manual Trade Only'))
        return True
    except Exception as e:
        log.warning('TELEGRAM IMAGE TIMER EDIT FAILED kind=%s message_id=%s: %s',kind,message_id,e); return False

async def signal_timer(cid,msg,signal):
    if msg is None: return
    total=signal.expiry*60; expiry_ts=signal.entry_ts+total
    while True:
        ...
        remaining=int(expiry_ts-time.time())
        if remaining<=0: break
        await edit_image(cid,msg.message_id,'signal',signal.asset,signal.direction,signal.confidence,signal.expiry,signal.reason,entry=f'{signal.entry:.6f}',technical='PASSED',signal_time=datetime.fromtimestamp(signal.entry_ts,UAE).strftime('%H:%M:%S UAE'),total_seconds=total,remaining=remaining,expiry_time=datetime.fromtimestamp(expiry_ts,UAE).strftime('%H:%M:%S UAE'))
        log.info('CANDICE SIGNAL TIMER asset=%s remaining=%s',signal.asset,remaining)

async def send_session_image(cid):
    start,end,next_start,active=session_window(); p=session_payload(); return await send_image(cid,'session',selected.get(cid,'SYSTEM'),**p)
...
async def cmd_assets(update,ctx):
    assets=await broker.live_assets(); markup=keyboard(assets); msg=await send_image(update.effective_user.id,'selected','FLEX',reply_markup=markup,caption='CANDICE AI • FLEX ASSET SELECTOR • Manual Trade Only',selected_time=now().strftime('%H:%M:%S UAE'))
    return msg
...
async def result_monitor(uid,signal,msg):
    expiry_ts=signal.entry_ts+signal.expiry*60
    await signal_timer(uid,msg,signal)
    reached=now().strftime('%H:%M:%S UAE')
    await send_image(uid,'expiry',signal.asset,direction=signal.direction,expiry=signal.expiry,entry=f'{signal.entry:.6f}',expiry_time=datetime.fromtimestamp(expiry_ts,UAE).strftime('%H:%M:%S UAE'),reached_time=reached)
    price=None; err=''
    for _ in range(5):
    ...
    if result=='LOSS':
        ...
        await send_image(uid,'recovery',signal.asset,recovery='15 MIN',quick_recovery='5 MIN',remaining=300,total_seconds=300,ratio=1,next_signal='After recovery check',daily_limit=f'{daily["losses"]}/{MAX_DAILY_LOSSES}')
        log.warning('CANDICE RECOVERY COOLDOWN asset=%s seconds=300',signal.asset)
...
async def scan_once():
    ...
    for uid,asset in list(selected.items()):
        ...
        if recovery_until.get(uid,0)>time.time():
            log.info('CANDICE RECOVERY WAIT pair=%s remaining=%ss',asset,int(recovery_until[uid]-time.time())); continue
        ...
        entry=float(df.close.iloc[-1]); entry_ts=time.time(); signal=Signal(asset,a.direction,a.confidence,a.expiry,entry,entry_ts,a.reason); active[uid]=signal; sent_keys.add(key)
        expiry_ts=entry_ts+a.expiry*60
        msg=await send_image(uid,'signal',asset,a.direction,a.confidence,a.expiry,a.reason,entry=f'{entry:.6f}',technical='PASSED',signal_time=datetime.fromtimestamp(entry_ts,UAE).strftime('%H:%M:%S UAE'),total_seconds=a.expiry*60,remaining=a.expiry*60,expiry_time=datetime.fromtimestamp(expiry_ts,UAE).strftime('%H:%M:%S UAE'))
