...
VERSION = "12.0-CANDICE-ADAPTIVE-READONLY"
AUTO_TRADE = False
MARTINGALE = False
...
def reset_risk():
    d = uae_date()
    if risk["date"] != d:
        risk.update(date=d, signals=0, wins=0, losses_total=0, losses=0, streak=0)
...
def normalize(raw):
    ...
    for x in raw[-300:]:
        if not isinstance(x, dict):
            continue
        o = f(x.get("open", x.get("o")))
        h = f(x.get("high", x.get("h")))
        l = f(x.get("low", x.get("l")))
        c = f(x.get("close", x.get("c")))
...
def adx(data, p=14):
    ...
    for i in range(p, len(tr) + 1):
        tv = sum(tr[i-p:i])
        if tv == 0:
            dirs.append((0.0, 0.0)); dx.append(0.0); continue
        pi, mi = 100 * sum(plus[i-p:i]) / tv, 100 * sum(minus[i-p:i]) / tv
        dirs.append((pi, mi))
        dx.append(100 * abs(pi - mi) / (pi + mi) if pi + mi else 0.0)
    if len(dx) < p:
        return None, None, None
    return sum(dx[-p:]) / p, dirs[-1][0], dirs[-1][1]
...
def aggregate(data, n):
    ...
    buckets = {}
    for x in data:
        minute = int(float(x["timestamp"])) // 60
        key = minute // n
    ...
    for key in sorted(buckets):
        ...
        if len(w) < n:
            continue
        out.append({"open": w[0]["open"], "high": max(x["high"] for x in w),
...
def timeframe_score(data):
    if len(data) < 25:
        return 0, 0, {"reason": "insufficient"}
    ...
    ax, dip, dim = adx(data)
    score = 0
    if e9 is not None and e21 is not None: score += 2 if e9 > e21 else -2
    if e21 is not None and e50 is not None: score += 2 if e21 > e50 else -2
    if rv is not None:
        if 52 <= rv < 72: score += 1
    ...
    return direction, score, {"EMA9": e9, "EMA21": e21, "EMA50": e50, "RSI": rv, "ADX": ax, "DI+": dip, "DI-": dim}
...
def analyze(data):
    if len(data) < 60:
        return {"decision": "NO_SIGNAL", "confidence": 0, "direction": None, "reasons": [f"warming history {len(data)}/60"]}
    c = [x["close"] for x in data]
    last, prev = data[-1], data[-2]
    e9, e21, e50 = ema(c, 9), ema(c, 21), ema(c, 50)
    rv, av = rsi(c), atr(data)
    ml, ms, md = macd(c)
    sk, ss = stochastic(data)
    ax, dip, dim = adx(data)
    support = min(x["low"] for x in data[-30:])
    resistance = max(x["high"] for x in data[-30:])
    score, reasons = 0, []
    if e9 and e21:
        score += 3 if e9 > e21 else -3; reasons.append("EMA9/21 bullish" if e9 > e21 else "EMA9/21 bearish")
    if e21 and e50:
        score += 2 if e21 > e50 else -2; reasons.append("trend bullish" if e21 > e50 else "trend bearish")
    if rv is not None:
    ...
    if ax is not None and ax >= 18:
        score += 2 if dip > dim else -2; reasons.append(f"ADX {ax:.1f} confirms {'bulls' if dip > dim else 'bears'}")
    if last["close"] > last["open"] and last["close"] >= prev["close"]: score += 1
    elif last["close"] < last["open"] and last["close"] <= prev["close"]: score -= 1
    ps, pat = pattern_signal(last, prev)
    score += 2 * ps
    ...
    d1, s1, _ = timeframe_score(data)
    d3, s3, _ = timeframe_score(aggregate(data, 3))
    d5, s5, _ = timeframe_score(aggregate(data, 5))
    if d1: score += 2 * d1
    if d3:
        score += 2 * d3; reasons.append("3m confirms" if d3 == d1 else "3m mixed")
    if d5:
        score += 2 * d5; reasons.append("5m confirms" if d5 == d1 else "5m mixed")

    direction = 1 if score > 0 else -1 if score < 0 else 0
    opposition = sum(1 for d in (d3, d5) if d and d == -d1)
    strong_entry = abs(s1) >= 5 and d1 != 0
    veto = strong_entry and opposition == 2 and abs(s3) >= 4 and abs(s5) >= 4
    confidence = int(max(0, min(95, 50 + min(42, abs(score) * 3.0))))
    quality = abs(s1) >= 4 and (d3 == d1 or d5 == d1 or abs(s1) >= 7)
    mtf = {"1m": s1, "3m": s3, "5m": s5}
    if veto:
        return {"decision": "NO_SIGNAL", "confidence": confidence, "direction": "UP" if d1 > 0 else "DOWN", "reasons": reasons[-7:] + ["strong 3m+5m opposition; setup rejected"], "mtf": mtf}
    if direction == 0 or not quality or confidence < MIN_CONFIDENCE:
        return {"decision": "NO_SIGNAL", "confidence": confidence, "direction": "UP" if direction > 0 else "DOWN" if direction < 0 else None, "reasons": reasons[-7:] + [f"adaptive quality gate: score={score}, 1m={s1}, 3m={s3}, 5m={s5}"], "mtf": mtf}
    return {"decision": "SIGNAL", "confidence": confidence, "direction": "UP" if direction > 0 else "DOWN", "reasons": reasons[-8:] + [f"adaptive MTF score {score}"], "mtf": mtf,
            "indicators": {"EMA9": e9, "EMA21": e21, "EMA50": e50, "RSI": rv, "ATR": av, "MACD": ml, "MACD_signal": ms, "MACD_hist": md, "StochK": sk, "StochD": ss, "ADX": ax, "DI+": dip, "DI-": dim, "support": support, "resistance": resistance, "pattern": pat}}
...
def telegram(text):
    ...
    try:
        ...
        log.warning("Telegram send failed: HTTP %s", getattr(e.response, "status_code", "?")); return False
...
def send_signal(asset, data, tech, expiry=5):
    ...
    expiry = min(EXPIRIES, key=lambda x: abs(x - int(expiry or 5)))
    ts, entry = data[-1]["timestamp"], data[-1]["close"]
    key = f"{asset}:{tech['direction']}:{expiry}:{int(ts // 60)}"
    if key in sent: return {"ok": True, "status": "DUPLICATE_BLOCKED"}
    sent.add(key); risk["signals"] += 1
    mtf, ind = tech.get("mtf", {}), tech.get("indicators", {})
    ...
    telegram(text)
    return {"ok": True, "status": "SIGNAL", "asset": asset, "direction": tech["direction"], "expiry": expiry, "confidence": tech["confidence"]}


def on_olymp_candle(asset, candle):
    with lock:
        q = candles.setdefault(asset, deque(maxlen=300))
        ts = int(candle.get("timestamp", time.time()))
        if last_minute_seen.get(asset) == ts: return
        q.append(candle); last_minute_seen[asset] = ts; data = list(q)
    tech = analyze(data)
    if tech["decision"] == "SIGNAL":
        result = send_signal(asset, data, tech, 5)
        log.info("DECISION asset=%s signal=%s dir=%s conf=%s mtf=%s", asset, result.get("status"), tech.get("direction"), tech.get("confidence"), tech.get("mtf"))
    else:
        log.info("DECISION asset=%s NO_SIGNAL reason=%s mtf=%s", asset, " | ".join(tech["reasons"][-2:]), tech.get("mtf"))
...
def health():
    ...
    return jsonify({"ok": True, "version": VERSION, "mode": "READ_ONLY_DEMO", "timeframe": "1m live ticks + derived 3m/5m", "expiries": list(EXPIRIES), "assets": list(candles.keys()), "telegram_configured": bool(TOKEN and CHAT_ID), "auto_trade": AUTO_TRADE, "martingale": MARTINGALE, "risk": risk.copy(), "brain": "adaptive multi-timeframe technical consensus", "olymp": s})
...
def tradingview_webhook():
    ...
    asset = str(p.get("asset", p.get("symbol", "UNKNOWN"))).upper().strip()
    raw = normalize(p.get("candles", p.get("bars", []))) or normalize([p])
    with lock:
        q = candles.setdefault(asset, deque(maxlen=300)); q.extend(raw); data = list(q)
    tech = analyze(data)
    return jsonify(send_signal(asset, data, tech, p.get("expiry", 5)) if tech["decision"] == "SIGNAL" else {"ok": True, "status": "NO_SIGNAL", "technical": tech})


def telegram_command_loop():
    global telegram_offset
    if not TOKEN or not CHAT_ID:
        log.warning("Telegram command listener disabled: credentials not configured"); return
    log.info("Telegram command listener started")
    while True:
        try:
            r = requests.get(f"https://api.telegram.org/bot{TOKEN}/getUpdates", params={"timeout": 25, "offset": telegram_offset + 1, "allowed_updates": json.dumps(["message"])}, timeout=35)
            if r.status_code == 401:
                log.error("Telegram token rejected (401). Replace TELEGRAM_BOT_TOKEN in Render; token value is never logged.")
                time.sleep(30); continue
            r.raise_for_status()
            for u in r.json().get("result", []):
                telegram_offset = max(telegram_offset, int(u.get("update_id", telegram_offset)))
                m = u.get("message") or {}; chat = str((m.get("chat") or {}).get("id", ""))
                if chat != CHAT_ID: continue
                text = str(m.get("text", "")).strip().lower()
                if text.startswith("/start"):
                    telegram("🎯 CANDICE AI 12.0\n\n✅ ONLINE\n📡 Olymp Trade LIVE READ-ONLY\n🕐 1m analysis + 3m/5m confirmation\n🧠 Adaptive full technical brain\n⏱️ 2 / 3 / 5 / 10 / 15 min\n🛡️ DEMO / MANUAL ONLY\n🚫 AUTO-TRADE OFF • MARTINGALE OFF\n\n/status • connection\n/performance • session")
            ...
            log.warning("Telegram polling error: %s", e); time.sleep(5)


@app.post("/result")
def result():
    p = request.get_json(silent=True) or {}; outcome = str(p.get("result", "")).upper(); reset_risk()
    if outcome == "WIN": risk["wins"] += 1; risk["streak"] = 0
    elif outcome == "LOSS": risk["losses_total"] += 1; risk["losses"] += 1; risk["streak"] += 1
    return jsonify({"ok": True, "risk": risk.copy()})
...
def main():
    ...
    reset_risk()
    try:
        live_feed = OlympLiveFeed(on_olymp_candle, seed_olymp_history)
        live_feed.start(); log.info("Olymp live feed started")
    except Exception as e: log.exception("Olymp live feed startup failed: %s", e)
    threading.Thread(target=telegram_command_loop, daemon=True, name="telegram").start()
