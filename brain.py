...
class CandiceBrain:
    ...
    def _load(self):
        try:
            ...
            x.setdefault("setups", {}); x.setdefault("assets", {}); x.setdefault("daily", {})
            return x
        except Exception:
    ...
    def _history_rate(self, key):
        x = self.learning["setups"].get(key, {"n": 0, "w": 0, "l": 0, "t": 0}); n = int(x.get("n", 0))
    ...
    def analyze(self, asset, completed, live_price):
        ...
        d = list(completed[-60:]); cl = [x["close"] for x in d]
        t1, s1 = self._trend(d[-30:]); all15 = self._aggregate_15m(completed[-360:]); t15, s15 = self._trend(all15, 5, 13) if len(all15) >= 13 else (0, 0)
        rsi = self._rsi(cl); adx = self._adx(d); atr = self._atr(d); macd = self._macd(cl); pat, pdir = self._pattern(d)
        support = min(x["low"] for x in d[-30:]); resistance = max(x["high"] for x in d[-30:]); up = down = 0; votes = []
        ...
        hi = max(x["high"] for x in d[-20:]); lo = min(x["low"] for x in d[-20:])
    ...
    def _message(self, s, target, now=None):
        st = self.feed.status(); now = time.time() if now is None else now
        local = datetime.fromtimestamp(now, timezone.utc).astimezone(UAE); tar = datetime.fromtimestamp(target, timezone.utc).astimezone(UAE)
        bal = st.get("account_balance"); b = "NOT_AVAILABLE" if bal is None else f"{bal:.2f} {st.get('account_currency', '')}".strip(); cd = max(0, int(target-now))
    ...
    def _result(self, s):
        ...
        if price is None:
            log.warning("RESULT_PRICE_UNAVAILABLE asset=%s", s["asset"]); return
        diff = price - s["entry"]; tolerance = max(abs(s["entry"])*1e-8, 1e-10)
        ...
        key = f"{s['asset']}|{s['direction']}|{s['pattern']}|{s['strategy']}|{s['trend_15']}|{s['expiry']}"; x = self.learning["setups"].setdefault(key, {"n":0,"w":0,"l":0,"t":0}); x["n"] += 1; x[{"WIN":"w","LOSS":"l","TIE":"t"}[result]] += 1
        self._reset_day_if_needed(); self.stats[result] += 1
        ...
        self.send(msg); log.info("RESULT asset=%s result=%s expiry=%s", s["asset"], result, s["expiry"])

    def _result_worker(self, s):
        time.sleep(max(0, s["expiry_start"] - time.time())); self._result(s)
    ...
    def on_candle(self, asset, candle):
        try:
            ...
            if scan:
                ...
                log.info("BRAIN_1M_ANALYSIS asset=%s direction=%s confidence=%s expiry=%s strategy=%s", asset, scan["direction"], scan["confidence"], scan["expiry"], scan["strategy"])
            else:
                self.last_scan.pop(asset, None); log.info("BRAIN_1M_ANALYSIS asset=%s decision=NO_SIGNAL", asset)
        except Exception:
            log.exception("BRAIN_CANDLE_ANALYSIS_FAILED asset=%s", asset)
        for s in list(self.pending.values()):
            if s["asset"] == asset and not s.get("result") and time.time() >= s["expiry_start"]: self._result(s)
    ...
    def run(self):
        while True:
            try:
                self._reset_day_if_needed()
                now = time.time(); target = (int(now // 300) + 1) * 300; decision_time = target - 40
                # Never issue a late signal.  If the process starts after the
                # 40-second window, skip that boundary and wait for the next one.
                if now > decision_time + 1:
                    target += 300; decision_time += 300
                time.sleep(max(0, decision_time - time.time()))
                ...
                if self._blocked():
                    ...
                    time.sleep(max(1, target - time.time())); continue
                if time.time() > decision_time + 2:
                    log.warning("SIGNAL_WINDOW_MISSED cycle=%s", cycle); time.sleep(max(1, target-time.time())); continue
                s = self._best_scan()
                if not s:
                    log.info("SIGNAL_WINDOW_NO_SIGNAL target=%s", target); time.sleep(max(1, target-time.time())); continue
                fresh = self.feed.live_price(s["asset"])
                if fresh is None:
                    log.warning("SIGNAL_ABORT reason=no_live_price asset=%s", s["asset"]); time.sleep(max(1, target-time.time())); continue
                ...
                if not isinstance(r, dict) or not r.get("message_id"):
                    # One attempt per exact signal window.  Do not retry every
                    # few seconds and accidentally move the entry timestamp.
                    log.warning("TELEGRAM_SIGNAL_NOT_CONFIRMED cycle=%s", cycle)
                    time.sleep(max(1, target-time.time())); continue
                self.sent_cycles.add(dedupe); s["telegram_message_id"] = r["message_id"]
                key = f"{s['asset']}|{s['direction']}|{s['expiry']}|{cycle}"; self.pending[key] = s
                ...
                threading.Thread(target=self._countdown, args=(s,target), daemon=True).start(); threading.Thread(target=self._result_worker, args=(s,), daemon=True).start()
                log.info("SIGNAL_CONFIRMED asset=%s direction=%s entry=%s signal=%s target=%s expiry=%s", s["asset"], s["direction"], s["entry"], datetime.fromtimestamp(s["signal_time"],timezone.utc).astimezone(UAE).strftime("%H:%M:%S"), datetime.fromtimestamp(target,timezone.utc).astimezone(UAE).strftime("%H:%M:%S"), s["expiry"])
                time.sleep(max(1, target-time.time()))
            except Exception:
                log.exception("BRAIN_LOOP_ERROR"); time.sleep(2)
