...
async def _candice_recovery(a, pair, df1, df5, tech):
    ...
    if not getattr(a, "OPENROUTER_API_KEY", ""):
        # Keep the normal minimum gate; without the provider, do not guess.
    ...
    prompt = (
        ...
        f"Pair: {pair}\n"
        f"Technical direction: {tech.get('signal')}\n"
        f"Technical confidence: {tech.get('confidence')}\n"
        f"5m trend: {tech.get('trend_5m')}\n"
        f"Technical reason: {tech.get('reason')}\n"
        f"Fresh 1m candles: {len(df1)}\n"
        f"Fresh 5m candles: {len(df5)}\n"
    )
    try:
        ...
        if confidence < RECOVERY_MIN_CONF:
            return None, f"recovery confidence {confidence}% below {RECOVERY_MIN_CONF}%"
        if duration not in RECOVERY_DURATIONS:
            return None, "recovery duration must be 1 or 2 minutes"
    ...
    while time.time() < deadline:
        try:
            ...
            if df1 is None or df5 is None:
                ...
                if tech.get("signal") == "NO SIGNAL":
                    last_reason = "mixed technical structure"
                elif tech.get("confidence", 0) < 70:
                    last_reason = "technical confidence below recovery threshold"
                elif str(tech.get("trend_5m", "")).upper() != str(tech.get("signal", "")).upper():
                    last_reason = "5m context conflicts"
                else:
                    ...
                    if ai:
                        return {
                            ...
                            "candle_time": a.fmt_ts(float(df1["timestamp"].iloc[-1])),
                            "ai_reason": "SHORT RECOVERY: " + ai["reason"],
                            "recovery": True,
                        }, None
...
async def _recovery_result(a, bot, uid, signal):
    ...
    a.active_signal.pop(uid, None)
    a.selected_asset.pop(uid, None)
    await a.send_asset_menu(bot, uid, "✅ Recovery cycle complete → AI will select a fresh asset.")
...
async def _patched_monitor_result(bot, uid, signal):
    ...
    if result == "LOSS" and not signal.get("recovery", False):
        ...
        if recovery:
            ...
            asyncio.create_task(_recovery_result(a, bot, uid, recovery))
            return
        a.selected_asset.pop(uid, None)
        await a.send_asset_menu(bot, uid, "⚠️ No safe short recovery setup → select a fresh asset.")
    else:
        a.selected_asset.pop(uid, None)
        await a.send_asset_menu(bot, uid, "🔁 Recovery complete → select a fresh asset.")


def _patch():
    global PATCHED
    a = _app()
    if not a or getattr(a, "_LOSS_RECOVERY_AI_V1", False):
        return bool(a)
