...
class OlympReadOnlyClient:
 ...
 async def close(self):
  ...
  for task in (getattr(self,'_reader_task',None),getattr(self,'_dispatcher_task',None)):
  ...
  try:
   while self.running:
    ...
    for msg in rows:
     ...
     e=msg.get("e");callbacks=list(self.callbacks.get(e,[]))+list(self.callbacks.get("*",[]))
     for cb in callbacks:
      try:
       r=cb(msg)
 ...
 async def initialize_read_only(self):
  ...
  self.account_mode="DEMO" if snapshots.get("demo") is not None else "UNKNOWN"
  if snapshots.get("demo") is not None:self.account_balance,self.account_currency=snapshots["demo"]
