...
def card(kind='selected',asset='ASIA_X',direction='',confidence=0,expiry=0,reason='',**kw):
 ...
 if kind=='selected':
  ...
  for icon,label,value,col in rows: tx(d,(70,y),icon,27,col,True,'lm'); tx(d,(125,y),label,21,MUTED,False,'lm'); tx(d,(810,y),value,21,col,True,'ra'); d.line((65,y+28,835,y+28),fill=(35,75,105,90),width=1); y+=82
  kv(d,70,770,'Selected At',kw.get('selected_time','—'),CYAN); tx(d,(450,840),'ASSET READY FOR CANDICE RESEARCH',23,GREEN,True,'mm'); tx(d,(450,885),'Research continues while this asset rests.',18,MUTED,False,'mm')
 elif kind=='signal':
  col=GREEN if direction=='UP' else RED if direction=='DOWN' else WHITE; tx(d,(60,355),direction or 'WAIT',72,col,True,'lm'); kv(d,60,455,'Entry',kw.get('entry','—')); kv(d,60,505,'Signal Time',kw.get('signal_time','—'),CYAN); kv(d,60,555,'Duration',f'{expiry} MIN'); kv(d,60,605,'Candice AI',f'APPROVED • {confidence}%',GREEN); kv(d,60,655,'Technical',kw.get('technical','PASSED'),CYAN)
  ...
  tx(d,(450,370),'EXPIRY REACHED',50,RED,True,'mm'); kv(d,60,470,'Direction',direction or '—',RED if direction=='DOWN' else GREEN); kv(d,60,525,'Entry',kw.get('entry','—')); kv(d,60,580,'Duration',f'{expiry} MIN'); kv(d,60,635,'Expiry Boundary',kw.get('expiry_time','—'),ORANGE); kv(d,60,690,'Reached At',kw.get('reached_time','—'),CYAN); tx(d,(450,775),'VERIFYING CLOSED EXPIRY CANDLE…',24,ORANGE,True,'mm'); tx(d,(450,825),'Please wait for verified WIN / LOSS.',20,MUTED,False,'mm')
 elif kind=='result':
  ...
  remain=max(0,int(kw.get('remaining',300))); total=max(1,int(kw.get('total_seconds',300))); kv(d,60,390,'Recovery Window',kw.get('recovery','15 MIN'),GREEN); kv(d,60,460,'Quick Recovery',kw.get('quick_recovery','5 MIN'),ORANGE); kv(d,60,530,'Recovery Countdown',fmt(remain),CYAN); bar(d,60,585,840,remain/total,CYAN); kv(d,60,660,'Next Signal',kw.get('next_signal','Automatic'),CYAN); kv(d,60,720,'Daily Loss Limit',kw.get('daily_limit','ACTIVE'),GREEN); tx(d,(450,800),'RECOVERY PROTECTION ACTIVE',30,GREEN,True,'mm')
 else:
